...
import { getAllTrips, getTripById } from "~/api/tripApi";
...
export const clientLoader = async ({ params }: LoaderFunctionArgs) => {
  ...
  const [trip, unsortedTrips] = await Promise.all([
    await getTripById(Number(tripId)),
    await getAllTrips(100, 0),
  ]);

  const trips = [...unsortedTrips.allTrips]
...
const TripDetail = ({ loaderData }: Route.ComponentProps) => {
  const { tripImageUrls } = loaderData.trip ?? {};

  let tripData;
  if (loaderData) tripData = parseTripData(loaderData?.trip?.tripDetail ?? "");
  const {
    name,
    duration,
    itinerary,
      ...
      <section className="container wrapper-md">
        ...
        {tripImageUrls && (
          <section className="gallery">
            {tripImageUrls.map((url: string, i: number) => (
              <img
                src={url}
                key={i}
                className={cn(
